package com.ike.sq.alliance.dao.base;
...
import com.ike.sq.alliance.dao.DaoMaster;
import com.ike.sq.alliance.dao.DaoSession;
import com.ike.sq.alliance.dao.MsgDao;
import com.ike.sq.alliance.dao.interfaces.IDatabase;
...
public abstract class BaseManager<M, K> implements IDatabase<M, K> {
    private static final String DEFAULT_DATABASE_NAME = "ike.db";
    ...
    @Override
    public List<M> loadPages(WhereCondition condition, int page, int number) {
        openReadableDb();
        return getAbstractDao().queryBuilder().where(condition)
                .offset(page * number).limit(number).list();
    }
     ...
     * @return
     */
    public List<M> loadGroup(WhereCondition condition) {
        openReadableDb();

        return getAbstractDao().queryBuilder().where(condition).orderDesc(MsgDao.Properties.Time).list();
    }
    ...
    @Override
    public long getPages(WhereCondition condition, int number) {
        long count = getAbstractDao().queryBuilder().where(condition).count();
        long page = count / number;
        if (page > 0 && count % number == 0) {
            return page - 1;
        }
